...
public class DIAAssignQuantUnit implements Runnable {
    ...
    public void run() {        
        ...
        if (ms1lcms.datattype != SpectralDataType.DataType.pSMART) {
            MatchFragment();
            pepIonID.RemoveRedundantFrag();
            if (pepIonID.FragmentPeaks.isEmpty() && Math.max(pepIonID.MaxProbability, Math.max(pepIonID.MS1AlignmentLocalProbability, pepIonID.MS2AlignmentLocalProbability)) > 0.8f) {
    ...
    private void MatchFragment() {
        for (Ion frag : pepIonID.GetFragments()) {
            ...
            for (PeakCluster peakCluster : pepIonID.MS1PeakClusters) {
                for (PrecursorFragmentPairEdge fragmentClusterUnit : peakCluster.Fragments) {
