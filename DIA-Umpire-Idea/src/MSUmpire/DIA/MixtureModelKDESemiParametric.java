...
public class MixtureModelKDESemiParametric {
    ...
    public void GeneratePlot(String pngfile) {
        ...
        MixtureModelProb = new Float[Binsize + 1][3];
        ...
        for (int i = 1; i < Binsize + 1; i++) {
            double positiveNumber = correct_kde_y[Binsize-i];
            double negativeNumber = decoy_kde_y[Binsize-i];
            MixtureModelProb[i][0] = (float) model_kde_x[Binsize-i];
            positiveaccu += positiveNumber;
            negativeaccu += negativeNumber;
            MixtureModelProb[i][2] = (float)(positiveNumber / (negativeNumber + positiveNumber));
            MixtureModelProb[i][1] = (float)(positiveaccu / (negativeaccu + positiveaccu));
        }
...
//        render.setSeriesStroke(2, new BasicStroke(2.0f));
    ...
    public void Modeling() {
        ...
        double miniIDscore=IDEmpiricalDist.getObs(0);
...
//        weight_correct = 0.2f;
    ...
    public void SetData(double[] targetdata, double[] decoydata, double[] iddata) {
        ...
        float intv = (max - min) / Binsize;
        model_kde_x = new double[Binsize];
        for (int i = 0; i < Binsize; i++) {
    ...
    private void GenerateIniCorrectDensity() {
        inicorrect_kde_y = new double[Binsize];
        for (int i = 0; i < Binsize; i++) {
    ...
    private void GenerateCorrectDensity() {
        correct_kde_y = new double[Binsize];
        for (int i = 0; i < Binsize; i++) {
    ...
    private void GenerateDecoyDensity() {
        decoy_kde_y = new double[Binsize];
        for (int i = 0; i < Binsize; i++) {
    ...
    private void GenerateTargetDensity() {
        model_kde_y = new double[Binsize];
        for (int i = 0; i < Binsize; i++) {
    ...
    private double IniCorrectKDELookUp(double x) {
        ...
        return inicorrect_kde_y[Binsize - 1];
    }
    ...
    private double CorrectKDELookUp(double x) {
        ...
        return correct_kde_y[Binsize - 1];
    }
