...
public class CurveCorrUnit implements Runnable {
    ...
    public void run() {
        ...
        if (peakA.StartRT() >= peakB.StartRT() && peakA.StartRT() <= peakB.FinalEndRT()) {
            overlap = true;
        } else if (peakA.FinalEndRT() >= peakB.StartRT() && peakA.FinalEndRT() <= peakB.FinalEndRT()) {
            overlap = true;
        } else if (peakB.StartRT() >= peakA.StartRT() && peakB.StartRT() <= peakA.FinalEndRT()) {
            overlap = true;
        } else if (peakB.FinalEndRT() >= peakA.StartRT() && peakB.FinalEndRT() <= peakA.FinalEndRT()) {
            overlap = true;
        }
        if (overlap) {
