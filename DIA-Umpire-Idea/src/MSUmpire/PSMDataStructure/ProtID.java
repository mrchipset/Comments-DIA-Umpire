...
public class ProtID implements Serializable{
    ...
    public void SetDescription(String Des) {
        ...
        if (Description.contains("GeneName=")) {
    ...
    public float GetAbundanceByAllFragment_IBAQ(float pepweight) {
     ...
     public int GetSILAC_H_Count() {
        int spc = 0;
        for (PepIonID pep : PeptideID.values()) {
            if (pep.ModSequence.contains("[6.02005(K)]")) {
    ...
    public int GetSILAC_L_Count() {
        int spc = 0;
        for (PepIonID pep : PeptideID.values()) {
            if (pep.ModSequence.contains("K") && !pep.ModSequence.contains("[6.02005(K)]")) {
                spc += pep.GetSpectralCount();
            }
        }
        return spc;
    ...
    public int GetSpectralCount() {
      ...
      public int GetFragCount() {
        int frag = 0;
        for (PepIonID pep : PeptideID.values()) {
            frag += pep.FragmentPeaks.size();
        }
        return frag;
    }

    public float GetAbundanceByMS1_IBAQ_SILAC_H() {
        float totalabundance = 0f;
        for (PepIonID peptide : PeptideID.values()) {
            if (peptide.PeakHeight != null && peptide.ModSequence.contains("[6.02005(K)]")) {
    ...
    public float GetAbundanceByMS1_IBAQ_SILAC_L() {
        float totalabundance = 0f;
        for (PepIonID peptide : PeptideID.values()) {
            if (peptide.PeakHeight != null && peptide.ModSequence.contains("K") && !peptide.ModSequence.contains("[6.02005(K)]")) {
                totalabundance += peptide.PeakHeight[0];
            }
        }
        return totalabundance / TheoPeptides.size();
    ...
    public float GetAbundanceByMS1_TopN(int topN, float pepweight) {
     ...
     public float GetPepAbundanceByTopAcrossSample(ArrayList<String> ProPep) {
        float totalabundance = 0f;
        if (ProPep != null) {
     ...
     public float GetPepAbundanceByTopAcrossSampleTopN(ArrayList<String> ProPep, int TopN) {
        float totalabundance = 0f;
        if (ProPep != null) {
            for (PepIonID pepIonID : PeptideID.values()) {
                if (ProPep.contains(pepIonID.GetKey())) {
    ...
    public float GetPepAbundanceByTopCorrFragAcrossSample(ArrayList<String> ProPep, HashMap<String, ArrayList<String>> PepFrag) {
        float totalabundance = 0f;
        int count=0;
        if (ProPep != null) {
            for (PepIonID pepIonID : PeptideID.values()) {
