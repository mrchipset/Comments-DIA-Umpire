...
public class PeakCluster implements Serializable {
    ...
    public ArrayList<PrecursorFragmentPairEdge> Fragments = new ArrayList<>();
        ...
        if (FragmentScan != null && !locked) {
            ...
            try {
                if (FragmentScan == null) {
                    ...
                    for (PrecursorFragmentPairEdge fragment : Fragments) {
    ...
    public float NeutralMass() {
        if (MonoIsotopePeak != null) {
            return Charge * (MonoIsotopePeak.TargetMz - 1.00727f);
        }
        return Charge * (mz[0] - 1.00727f);
    }
    ...
    public void CalcPeakArea_V2() {
        ...
        startRT = Math.max(MonoIsotopePeak.StartRT(), IsoPeaksCurves[1].StartRT());
        endRT = Math.min(MonoIsotopePeak.EndRT(), IsoPeaksCurves[1].EndRT());
